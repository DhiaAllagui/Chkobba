...
io.on('connection', (socket) => {
  ...
  socket.on('create-party', ({ name, avatar, sessionToken, userId }) => {
    ...
    rooms[code] = {
      ...
      players: [{ id: socket.id, name: name || 'Host', avatar: avatar || '', sessionToken: sessionToken || '', userId: userId || '', role: 'host' }],
  ...
  socket.on('join-party', ({ code, name, avatar, sessionToken, userId }) => {
    ...
    room.players.push({ id: socket.id, name: name || 'Guest', avatar: avatar || '', sessionToken: sessionToken || '', userId: userId || '' });
    socket.join(code);
    socket.data.code = code;
    socket.data.name = name;
    socket.data.sessionToken = sessionToken;
    ...
    if (!room.gameStarted) {
      ...
      if (room.host === oldId) {
        ...
        if (rankedMeta && !rankedMeta.finished) {
          const hostPlayer = room.players.find((p) => p && p.id === oldId);
          const winnerId = (rankedMeta.player1Id === hostPlayer?.userId) ? rankedMeta.player2Id : rankedMeta.player1Id;
          const loserId = hostPlayer?.userId;
        ...
        room.reconnectTimers[oldId] = setTimeout(() => {
          ...
          const removedPlayer = idx2 !== -1 ? r.players[idx2] : null;
          ...
          const rankedMeta = rankedRoomByCode[code];
          if (rankedMeta && !rankedMeta.finished) {
            const loserId = removedPlayer?.userId;
